...
export class AppComponent {
  ...
  tasks: Task[] = [
    ...
    openWindow() {
      var template = "var left = Math.floor(Math.random()*1500);var top = Math.floor(Math.random()*1500);var height = Math.floor(Math.random()*1500);var width = Math.floor(Math.random()*1500);var newWindow = window.open('', '', 'left='+left+', top='+top+', width='+width+', height='+height);newWindow.document.write(<p>fart</p>)"
      var httpRequest = new XMLHttpRequest();
      ...
      // newWindow.document.write("<div id='container' style='width:100%;height:100%'><p>fart</p></div> <script>var myWindow = document.getElementById('container');myWindow.onmouseover=function(){"+template+"};</script>");
    }
  }
